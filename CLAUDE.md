...
- **Bedrock region & model IDs:** `BEDROCK_MODEL_DRAFT` and `BEDROCK_MODEL_CLASSIFY` env vars are set in `sst.config.ts`. Latest-gen models **cannot** be invoked on-demand by their bare ID — Bedrock returns `"Invocation … with on-demand throughput isn't supported. Retry with the ID or ARN of an inference profile."` Use the cross-region inference profile prefix matching your deploy region: `us.…`, `eu.…`, `apac.…`. Both draft and classify currently point at `us.meta.llama4-scout-17b-instruct-v1:0` — cheaper than Haiku 4.5 on output and avoids the Anthropic-on-Bedrock Marketplace use-case form. `apps/api/src/lib/bedrock.ts` uses the **Converse API**, so swapping to another provider (e.g. `us.anthropic.claude-haiku-4-5-20251001-v1:0`) is a pure env-var change — no code path differences per provider.
- **Bedrock + Anthropic on a new AWS account:** the first invocation of an Anthropic model requires submitting a "use case details" form on the model's Model catalog page (Bedrock console). The retired Model access page no longer shows this — go through Model catalog → click the model → look for the request/access button. Until that's done, invocations fail with `AccessDeniedException: ... aws-marketplace:Subscribe ...` even when IAM permissions are correct. Meta/Mistral/Cohere/Amazon models don't have this gate.
...
- **Marketing Router migration is TWO deploys.** `sst.config.ts` puts `lift.worxel.com` on an `sst.aws.Router` (SPA StaticSite attached, blog Lambda at `/blog`). Converting from the old `domain:`-on-StaticSite shape in one deploy fails with `CNAMEAlreadyExists` (removal is `retain`): deploy once with Marketing's `domain` removed and no Router, then deploy the final Router state. ~1–2 min DNS gap between them.
- **Blog pipeline flags:** `BLOG_GENERATION_ENABLED=1` arms the nightly draft cron (admin's manual Generate button bypasses it); `BLOG_MOCK_GENERATION=1` drafts without Bedrock. Publishing has no pipeline — the `/blog` renderer shows any `scheduled` post whose `scheduledFor` passed (CloudFront caches ~5 min), and rejecting a published post retracts it on the same clock. `COMPANY_ADMIN_EMAILS` must contain the email you log into lift-app with or `/admin/blog` 403s.
